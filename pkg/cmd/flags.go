...
func AddWaitFlag(cmd *cobra.Command, wait *kube.WaitStrategy) {
	cmd.Flags().Var(
		...
		"wait",
		"wait until resources are ready (up to --timeout). Use '--wait' alone for 'watcher' strategy, or specify one of: 'watcher', 'hookOnly', 'legacy', 'ordered'. 'ordered' enables DAG-based subchart sequencing (HIP-0025). Default when flag is omitted: 'hookOnly'.",
	)
